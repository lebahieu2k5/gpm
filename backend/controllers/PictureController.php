...
class PictureController extends Controller
{
    ...
    {    
        ...
        Yii::$app->session['catts']=$_GET['catid'];
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                if(!is_null($file))
                {
                    $model->image = \func::taoduongdan(time() ."-" .$file->name);
                }
                else
                    $model->image=$oldfile;
                if ($model->save()){
                    ...
                    // upload anh
                    if (!is_null($file)) {
                        $path = dirname(dirname(__DIR__)) . '/images/picture/'.$model->image;
                        $file->saveAs($path);

                        $oldpath = dirname(dirname(__DIR__)) . '/images/picture/'.$oldfile;
                        if (is_file($oldpath))
                            unlink($oldpath);
                    }
                    return [
    ...
    /**
     * Delete an existing Picture model.
     * For ajax request will return json object
    ...
    public function actionDelete($id)
    {
     ...
     /**
     * Delete multiple existing Picture model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $id
     * @return mixed
    ...
    {
        $data = Picture::find()->where(['id'=>$_POST['id']])->one();
        Picture::updateAll(['home'=>(1-$data->home)],['id'=>$data->id]);
    }
    public function actionUpdateord()
    {
        $data = Picture::find()->where(['id'=>$_POST['id']])->one();
        Picture::updateAll(['ord'=>$_POST['value']],['id'=>$data->id]);
    }
    public function actionRedirects(){
        return $this->redirect(['picture/index','catid'=>$_GET['catid']]);
    }
}
