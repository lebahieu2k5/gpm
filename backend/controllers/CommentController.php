...
class CommentController extends Controller
{
    ...
    {
        $data = Comment::find()->where(['id'=>$_POST['id']])->one();
        Comment::updateAll(['active'=>(1-$data->active)],['id'=>$data->id]);
    }

    public function actionUpdateord()
    {
        $data = Comment::find()->where(['id'=>$_POST['id']])->one();
        Comment::updateAll(['ord'=>$_POST['value']],['id'=>$data->id]);
    }
